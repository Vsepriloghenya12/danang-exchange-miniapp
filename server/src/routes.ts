...
export function createApiRouter(opts: {
  ...
  router.get("/rates/today", (_req, res) => {
    ...
    res.json({ ok: true, date: today, data, pairMarkups: store.config?.pairMarkups || { USDT_USD: { buy: 0, sell: 0 } } });
  });
  ...
  router.post("/admin/rates/today", (req, res) => {
    try {
      ...
      const pairMarkups = body.pairMarkups || body.config?.pairMarkups || {};
      ...
      const pairBuy = Number(pairMarkups?.USDT_USD?.buy ?? 0);
      const pairSell = Number(pairMarkups?.USDT_USD?.sell ?? 0);

      const store = readStore();
      store.config = store.config || {};
      store.config.pairMarkups = {
        ...(store.config.pairMarkups || {}),
        USDT_USD: {
          buy: Number.isFinite(pairBuy) ? pairBuy : 0,
          sell: Number.isFinite(pairSell) ? pairSell : 0
        }
      };
      ...
      writeStore(store);
      res.json({ ok: true, date: today, data: store.ratesByDate[today], pairMarkups: store.config?.pairMarkups || { USDT_USD: { buy: 0, sell: 0 } } });
    } catch (e: any) {
      res.status(401).json({ ok: false, error: e?.message || "auth_failed" });
    }
  });
  ...
  router.post("/requests", async (req, res) => {
    try {
      ...
      const allowedCur = new Set<Currency>(["RUB", "USD", "USDT", "EUR", "THB", "VND"]);
      ...
      const isIntMultiple = (n: number, step: number) => Number.isFinite(n) && Math.round(n) % step === 0;

      if (buyCurrency === "VND" && receiveMethod === "atm" && !isIntMultiple(buyAmount, 100_000)) {
        return res.status(400).json({ ok: false, error: "bad_vnd_multiple", hint: "VND через банкомат выдаём только кратно 100000" });
      }

      if ((sellCurrency === "THB" && !isIntMultiple(sellAmount, 100)) || (buyCurrency === "THB" && !isIntMultiple(buyAmount, 100))) {
        return res.status(400).json({ ok: false, error: "bad_thb_multiple", hint: "THB принимаем и выдаём только кратно 100" });
      }
