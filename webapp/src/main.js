...
try {
    if (typeof window !== "undefined") {
        ...
        lockViewport();
        setVh();
        window.addEventListener("resize", resetViewportState, { passive: true });
        window.addEventListener("orientationchange", resetViewportState, { passive: true });
        window.visualViewport?.addEventListener("resize", resetViewportState, { passive: true });
        const preventGestureZoom = (ev) => {
            ev.preventDefault();
            lockViewport();
        };
        ...
        window.addEventListener("wheel", (ev) => {
            if (!(ev instanceof WheelEvent) || !ev.ctrlKey)
                return;
            ev.preventDefault();
        }, { passive: false });
        const isTextControl = (el) => el instanceof HTMLInputElement || el instanceof HTMLTextAreaElement || el instanceof HTMLSelectElement;
        const handleFocusIn = (ev) => {
            if (!isTextControl(ev.target))
                return;
