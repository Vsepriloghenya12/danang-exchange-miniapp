  ...
  if (!list.length) {
    return (
      <div className="card" style={{ padding: 14 }}>
        ...
        <div className="small">Пока нет вопросов.</div>
      </div>
    );
  }
  ...
  return (
    <div className="mx-faq">
      {list.map((it) => {
        ...
        return (
          <div key={it.id} className={open ? "mx-faqItem is-open" : "mx-faqItem"}>
            <button
              type="button"
              className="mx-faqQ"
              onClick={() => setOpenId((x) => (x === it.id ? "" : it.id))}
            >
              <span>{it.q}</span>
              <span className="mx-faqChevron">{open ? "—" : "+"}</span>
            </button>
            {open ? <div className="mx-faqA">{it.a}</div> : null}
          </div>
        );
      })}
    </div>
